...
export default function UpdateCourse() {
  ...
  useEffect(()=>{
    try {
        axios(
          `http://localhost:4000/getPerticularCourse/${id}`,{
          headers: {
    ...
    if(validate(page)){
      ...
      try {
        coursePage = await axios({
          url: "http://localhost:4000/updateCourseOverview",
          method: 'POST',
          Headers: {
              'content-Type': 'multipart/form-data',
          },
        ...
        for(let j=0; j<sectionField[i].lectureNo;j++){
          ...
          try {
             response = await axios({
              url: 'http://localhost:4000/updateSectionContent',
              method: 'POST',
              Headers: {
                  'content-Type': 'multipart/form-data',
              },
    ...
    if(sectionExist){
      ...
      try {
        landingPage = await axios({
          url: 'http://localhost:4000/updateLandingPage',
          method: 'POST',
          Headers: {
              'content-Type': 'multipart/form-data',
          },
